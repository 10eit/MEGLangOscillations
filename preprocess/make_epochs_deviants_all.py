...
for subject in subject_list:
    ...
    os.makedirs(op.join(deriv_root,'methods/all_deviant_response'),exist_ok=True)
    epoch_root = op.join(deriv_root,'methods/all_deviant_response')
    ...
    for run in runs[session]:
        ...
        three_reg = (beh_data['length'] == 3) & (beh_data['word_deviant']=='n') & ~(beh_data['response'].isin([1, -1]))
        three_omi = (beh_data['length'] == 3) & (beh_data['omission']==1) & (beh_data['response'] == 1)
        three_rev = (beh_data['length'] == 3) & (beh_data['reversal']==1) & (beh_data['response'] == 1)
        three_sem = (beh_data['length'] == 3) & (beh_data['word_deviant']=='sem') & (beh_data['response'] == -1)
        three_ambg = (beh_data['length'] == 3) & (beh_data['word_deviant']=='both') & (beh_data['response'] != 0)

        four_reg = (beh_data['length'] == 4) & (beh_data['word_deviant']=='n') & ~(beh_data['response'].isin([1, -1]))
        four_omi = (beh_data['length'] == 4) & (beh_data['omission']==1) & (beh_data['response'] == 1)
        four_rev = (beh_data['length'] == 4) & (beh_data['reversal']==1)
        four_sem = (beh_data['length'] == 4) & (beh_data['word_deviant']=='sem') & (beh_data['response'] == -1)
        four_ambg = (beh_data['length'] == 4) & (beh_data['word_deviant']=='both') & (beh_data['response'] != 0)

        raw = mne.io.read_raw_fif(bids_path, preload=True).notch_filter([50, 100],n_jobs=12).filter(1,45,n_jobs=12)
        ...
        corrected_events = events_delay.copy() # aligned with sound offset
        
        corrected_events[three_reg,2]=501
        corrected_events[three_omi,2]=502
        corrected_events[three_rev,2]=503
        corrected_events[three_sem,2]=504
        corrected_events[three_ambg,2]=505

        corrected_events[four_reg,2]=601
        corrected_events[four_omi,2]=602
        corrected_events[four_rev,2]=603
        corrected_events[four_sem,2]=604
        corrected_events[four_ambg,2]=605

        trials_mapping={'three_reg':501, 'three_omi':502, 'three_rev':503, 'three_sem': 504, 'three_ambg': 505,
                        'four_reg':601, 'four_omi':602, 'four_rev':603, 'four_sem': 604, 'four_ambg': 605}
