...
for subject in subject_list[-2:]:
    ...
    os.makedirs(op.join(deriv_root,'methods/rsa_data'),exist_ok=True)
    epoch_root = op.join(deriv_root,'methods/rsa_data')
    ...
    for run in runs[session]:
        ...
        conflict_str = (beh_data['word_deviant']=='both') & (beh_data['response'] == 1)
        conflict_sem = (beh_data['word_deviant']=='both') & (beh_data['response'] == -1)
